# sync-shared-sprint4b.ps1
# Rebru -- Shared File Sync (Sprint 4B Update)
# Menyinkronisasi 18 file canonical dari Website ke Bio-link
#
# CHANGELOG Sprint 4B:
#   + services\order-supabase.ts  -- pencatatan order ke Supabase
#   CartDrawer.tsx sudah include Sprint 4B changes
...
$SharedFiles = @(
  ...
  @{ From = "services\order-supabase.ts";        To = "services\order-supabase.ts"        },
...
$header = "
============================================================
  REBRU SYNC (Sprint 4B) $modeLabel -- $timestamp
  Dari : $WebsiteRoot
  Ke   : $BioLinkRoot
...
if (-not (Test-Path $WebsiteRoot)) {
  ...
  Write-Host $msg -ForegroundColor Red; Add-Content -Path $LogFile -Value $msg; exit 1
}
...
foreach ($file in $SharedFiles) {
  ...
  if (-not (Test-Path $src)) {
    ...
    Write-Host $msg -ForegroundColor Yellow; Add-Content -Path $LogFile -Value $msg; $skipped++; continue
  }
  if (-not (Test-Path $dstDir)) {
    if (-not $DryRun) { New-Item -ItemType Directory -Path $dstDir -Force | Out-Null }
    $msg = "  [MKDIR] Buat folder: $($dstDir.Replace($BioLinkRoot, 'src'))"
    Write-Host $msg -ForegroundColor DarkCyan; Add-Content -Path $LogFile -Value $msg
  }
  if (Test-Path $dst) {
    ...
    if ($dstTime -gt $srcTime) {
      ...
      Write-Host $warnMsg -ForegroundColor Yellow; Add-Content -Path $LogFile -Value $warnMsg; $warned++
    }
  }
  try {
    ...
    Write-Host $msg -ForegroundColor Green; Add-Content -Path $LogFile -Value $msg; $ok++
  } catch {
    $errMsg = "  [ERROR] $($file.From) -- $($_.Exception.Message)"
    Write-Host $errMsg -ForegroundColor Red; Add-Content -Path $LogFile -Value $errMsg; $errors++
  }
}
...
------------------------------------------------------------
  ...
  2. npm run dev            (test checkout + cek tabel orders)
  3. git add . && git commit -m sprint4b && git push
============================================================"
